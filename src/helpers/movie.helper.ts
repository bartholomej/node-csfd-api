...
export const getParent = (el: HTMLElement): CSFDParent | null => {
  ...
  return {
    series: {
      id: parseIdFromUrl(parentSeries?.getAttribute('href') || ''),
      name: parentSeries?.textContent?.trim() || null
    },
    season: {
      id: parseIdFromUrl(parentSeason?.getAttribute('href') || ''),
      name: parentSeason?.textContent?.trim() || null
    }
  };
};
